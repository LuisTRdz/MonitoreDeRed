...
function generarNombreArchivo(tipo, servidor) {
    const inicio = getFechaInicioSemana();
...
function parseLinea(linea, tipoArchivo) {
    ...
    if (tipoArchivo === "status") {
        ...
        // por seguridad, asumimos orden status
        fechaHora = partes[0];
        servidor = partes[1];
        estado = partes[2];
    }
...
// Función para ordenar los datos por fecha descendente (más recientes primero)
...
// Función para ordenar los datos por fecha ascendente (más antiguos primero)
...
// Función de filtrado por fecha y hora
function filtrarDatos(datos, fechaFiltro, horaFiltro) {
    return datos.filter(d => {
        ...
        if (horaFiltro) {
            const horaDato = (d.fecha.split(" ")[1] || "00:00:00").split(":")[0]; // extrae solo HH
...
function mostrarModalExportacion(tipo, servidor) {
    ...
    modal.classList.add("export-modal"); // ya lo estás haciendo

    // Quita los estilos en línea conflictivos
...
`;
    ...
    // Si el body tiene dark-mode, aplica también al modal
    ...
    document.getElementById("btn-confirm-export").addEventListener("click", () => {
        ...
        let datosFiltrados = filtrarDatosPorRango(
...
function formatearFecha(fechaOriginal) {
    // fechaOriginal: "14-08-2025 08:52:50"
...
function filtrarDatosPorRango(datos, fechaInicio, horaInicio, fechaFin, horaFin) {
    const inicio = fechaInicio ? new Date(`${fechaInicio}T${horaInicio?.padStart(2, '0') || '00'}:00:00`) : null;
    const fin = fechaFin ? new Date(`${fechaFin}T${horaFin?.padStart(2, '0') || '23'}:59:59`) : null;

    return datos.filter(d => {
        // d.fecha viene en "DD-MM-YYYY HH:mm:ss"
        const [dia, mes, anio] = d.fecha.split(" ")[0].split("-");
        const hora = d.fecha.split(" ")[1] || "00:00:00";
        const fechaDato = new Date(`${anio}-${mes}-${dia}T${hora}`);
...
function crearTablaConFiltros(datosCompletos, contenedor, tipo, onFiltrar) {
    ...
    btnLimpiar.style.backgroundColor = "#8a3434ff";
    ...
    btnLimpiar.addEventListener("mouseleave", () => btnLimpiar.style.backgroundColor = "#8a3434ff");
...
function agruparDatos(datos, maxPuntos = 50) {
    if (datos.length <= maxPuntos) return datos; // No agrupamos si hay pocos datos

    const grupoSize = Math.ceil(datos.length / maxPuntos);
    const datosAgrupados = [];

    for (let i = 0; i < datos.length; i += grupoSize) {
        const grupo = datos.slice(i, i + grupoSize);

        // Tomamos la fecha del punto medio del grupo
        const fecha = grupo[Math.floor(grupo.length / 2)].fecha;

        // Filtramos valores nulos para calcular promedio
        const latenciasValidas = grupo
            .map(d => d.latencia)
            .filter(v => typeof v === 'number');

        // Si no hay valores válidos, dejamos null
        const latencia = latenciasValidas.length > 0
            ? Math.round(latenciasValidas.reduce((sum, v) => sum + v, 0) / latenciasValidas.length)
            : null;

        datosAgrupados.push({ fecha, latencia });
    }
...
async function renderServidor(servidor) {
    ...
    const archivoStatus = generarNombreArchivo("status", servidor);
    const archivoProblemas = generarNombreArchivo("problemas", servidor);

    try {
        datosStatusCompletos = await cargarArchivoTxt(archivoStatus, "status");
        datosProblemasCompletos = await cargarArchivoTxt(archivoProblemas, "problemas");

        // Igualar latencia en problemas si falta
        datosProblemasCompletos = datosProblemasCompletos.map(p => {
            if (p.latencia === null) {
                const match = datosStatusCompletos.find(s => s.fecha === p.fecha && s.servidor === p.servidor && s.latencia !== null);
        ...
        async function actualizarDatosSolo() {
            try {
                const nuevosStatus = await cargarArchivoTxt(archivoStatus, "status");
                const nuevosProblemas = await cargarArchivoTxt(archivoProblemas, "problemas");

                // Igualar latencia en problemas
                const problemasConLat = nuevosProblemas.map(p => {
                    if (p.latencia === null) {
                        const match = nuevosStatus.find(s => s.fecha === p.fecha && s.servidor === p.servidor && s.latencia !== null);
                        if (match) return { ...p, latencia: match.latencia };
                    }
                    return p;
                });

                datosStatusCompletos = nuevosStatus;
                datosProblemasCompletos = problemasConLat;
